---
title: "mlr3learners.lightgbm: Mulitclass Classification Example"
date: "`r Sys.Date()`"
output:
...
In order to use the `mlr3learners.lightgbm` R package, please make sure, the reticulate package is configured properly on your system (reticulate version >= 1.13.0.9006) and is pointing to a python environment. If not, you can e.g. install `miniconda`:
...
```
```{r results='hide', message=FALSE, warning=FALSE, error=FALSE}
filename <- "./png/mlr3learners.lgb_imp_multiclass.png"
grDevices::png(
    filename = filename,
    res = 150,
    height = 1000,
